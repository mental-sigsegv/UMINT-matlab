...
%       Interval = min(30, max(floor(A1/doba_prechodu*2.5), 5));
Interval = floor(evalfis(f, [A1, A2, A3]));
    ...
    %% zelena na ceste A2
     ...
     % spocitanie aut z pruhov do cyklov
      ...
      [A1,A2,A3] = Ziskaj_poc_aut(pocet_aut_na_kriz);
%        Interval = min(30, max(floor(A2/doba_prechodu*2.5*1.5), 5));
Interval = floor(evalfis(f, [A2, A3, A1]));
   ...
   % spocitanie aut z pruhov do cyklov
      ...
      [A1,A2,A3] = Ziskaj_poc_aut(pocet_aut_na_kriz);
%        Interval = min(30, max(floor(A3/doba_prechodu*2.5), 5));
Interval = floor(evalfis(f, [A3, A1, A2]));
